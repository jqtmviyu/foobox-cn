...
/* if(m_y > brw.y && m_y < brw.y + brw.h) {
	...
	this.context_menu = function(x, y, id) {
		...
		if(foo_playcount) _autoplaylist.AppendMenuItem(MF_STRING, 204, "未播放过的音轨");
		if(foo_playcount) _autoplaylist.AppendMenuItem(MF_STRING, 205, "历史记录 (一个星期内播放过的音轨)");
		if(foo_playcount) _autoplaylist.AppendMenuItem(MF_STRING, 206, "最常播放的音轨");
		if(foo_playcount) _autoplaylist.AppendMenuItem(MF_STRING, 210, "最近添加的音轨");
