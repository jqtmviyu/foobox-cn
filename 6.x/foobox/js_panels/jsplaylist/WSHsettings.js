...
oPage = function(id, objectName, label, nbrows) {
	...
	this.init = function() {
		...
		case 3:
			...
			this.elements.push(new oCheckBox(12, 20, cSettings.topBarHeight + rh * 10.25, "无窗口边框", "ui_noborder ? true : false", "settings_checkboxes_action", this.id));
			this.elements.push(new oCheckBox(13, zoom(150, zdpi), cSettings.topBarHeight + rh * 10.25, "显示全屏按钮", "btn_fullscr ? true : false", "settings_checkboxes_action", this.id));
			this.elements.push(new oCheckBox(14, 20, cSettings.topBarHeight + rh * 11.25, "专辑图片=封面+碟片, 并以碟片优先 (适用于封面集成在碟片标签，会增加资源消耗)", "album_front_disc ? true : false", "settings_checkboxes_action", this.id));
			this.elements.push(new oCheckBox(15, 20, cSettings.topBarHeight + rh * 12.25, "右栏面板总是跟随光标 (不推荐)", "follow_cursor ? true : false", "settings_checkboxes_action", this.id));
			this.elements.push(new oCheckBox(16, 20, cSettings.topBarHeight + rh * 13.25, "右栏面板跟随主面板联动切换", "auto_sw ? true : false", "settings_checkboxes_action", this.id));
			this.elements.push(new oCheckBox(17, 20, cSettings.topBarHeight + rh * 14.25, "顶栏和底栏开启阴影效果", "show_shadow ? true : false", "settings_checkboxes_action", this.id));
			this.elements.push(new oRadioButton(18, 20, cSettings.topBarHeight + rh * 16.25, "系统", (sys_scrollbar == true), "settings_radioboxes_action", this.id));
			this.elements.push(new oRadioButton(19, zoom(120, zdpi), cSettings.topBarHeight + rh * 16.25, "较窄", (sys_scrollbar == false), "settings_radioboxes_action", this.id));
			if(foo_playcount){
				this.elements.push(new oRadioButton(20, 20, cSettings.topBarHeight + rh * 18.25, "同时写入文件标签及playcount记录", (rating2tag == true), "settings_radioboxes_action", this.id));
				this.elements.push(new oRadioButton(21, zoom(280, zdpi), cSettings.topBarHeight + rh * 18.25, "仅由playcount记录", (rating2tag == false), "settings_radioboxes_action", this.id));
			}
			break;
		case 4:
			//jssb options
	...
	this.draw = function(gr) {
		...
		case 3:
			...
			gr.gdiDrawText("其他选项", g_font_b, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 9.5 - (this.offset * cSettings.rowHeight), txt_width, p.settings.lineHeight, lc_txt);
			gr.gdiDrawText("滚动条宽度", g_font_b, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 15.5 - (this.offset * cSettings.rowHeight), txt_width, p.settings.lineHeight, lc_txt);
			gr.gdiDrawText("评级数据方案", g_font_b, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 17.5 - (this.offset * cSettings.rowHeight), txt_width, p.settings.lineHeight, lc_txt);
			if(!foo_playcount) gr.gdiDrawText("未安装 playcount 组件，评级数据将写入文件标签", g_font, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 18.25 - (this.offset * cSettings.rowHeight), txt_width, p.settings.lineHeight, lc_txt);
